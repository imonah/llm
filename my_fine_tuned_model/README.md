...
widget:
- source_sentence: Что делать при баге в проде?
  sentences:
  - Инструкция по смене пароля
  - Определение и пример smoke-теста
  - Алгоритм действий при баге в продакшене
- source_sentence: Как получить доступ к логам?
  sentences:
  - Инструкция по проверке выполнения тестов
  - Описание ошибки доступа 403
  - Инструкция по получению логов
...
print(similarities)
# tensor([[1.0000, 0.9255, 0.8205],
#         [0.9255, 1.0000, 0.9051],
#         [0.8205, 0.9051, 1.0000]])
```
...
* Approximate statistics based on the first 10 samples:
  ...
  |:--------|:----------------------------------------------------------------------------------|:----------------------------------------------------------------------------------|
  | type    | string                                                                            | string                                                                            |
  | details | <ul><li>min: 16 tokens</li><li>mean: 22.8 tokens</li><li>max: 30 tokens</li></ul> | <ul><li>min: 23 tokens</li><li>mean: 28.7 tokens</li><li>max: 35 tokens</li></ul> |
* Samples:
  | sentence_0                                | sentence_1                                    |
  |:------------------------------------------|:----------------------------------------------|
  | <code>Что такое smoke тест?</code>        | <code>Определение и пример smoke-теста</code> |
  | <code>Что делать при ошибке входа?</code> | <code>Решение проблем при авторизации</code>  |
  | <code>Как получить доступ к логам?</code> | <code>Инструкция по получению логов</code>    |
